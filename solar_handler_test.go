...
func TestHandleSolarMessage_ParsesAndRoutesFields(t *testing.T) {
	...
	written := map[string]InfluxMessage{}
	mockWrite := func(bucket string, msg InfluxMessage) {
		written[bucket] = msg
	}

	var solar solarMessage
	if err := json.Unmarshal(payloadBytes, &solar); err != nil {
		t.Fatalf("failed to unmarshal solar message: %v", err)
	}
	...
	for bucket, fields := range buckets {
		...
		mockWrite(bucket, InfluxMessage{
	...
	for _, bucket := range []string{"latest_energy", "latest_energy_current", "latest_voltage_current", "huis_sensors"} {
	...
	if _, ok := written["huis_sensors"].Fields["temp_sink_c"]; !ok {
		t.Error("expected temp_sink_c in huis_sensors bucket")
	}
	...
	if _, ok := written["latest_energy"].Tags["status"]; !ok {
		t.Error("expected tag 'status' to be present")
	}
